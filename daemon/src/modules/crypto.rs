// Copyright (c) 2026 DeltaIQx LLP. All rights reserved.`n// This software is proprietary and confidential.`n//! Cryptography module - Simple XOR encryption
...
pub fn decrypt_baseline(encrypted: &Vec<u8>) -> Result<String, String> {
    let key = get_master_key();
...
pub fn decrypt_consent(encrypted: &Vec<u8>) -> Result<String, String> {
    let key = get_master_key();
    let decrypted = decrypt_data(encrypted, &key)?;
    String::from_utf8(decrypted).map_err(|_| "Invalid UTF-8".to_string())
}
...
pub fn decrypt_event(encrypted: &Vec<u8>) -> Result<String, String> {
    let key = get_master_key();
    let decrypted = decrypt_data(encrypted, &key)?;
    String::from_utf8(decrypted).map_err(|_| "Invalid UTF-8".to_string())
}
